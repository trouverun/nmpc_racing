...
class DataCollector:
    def __init__(self, dataset_name="dataset"):
        ...
        if not os.path.exists("car_data"):
            os.mkdir("car_data")
        if not os.path.exists("car_data/%s" % dataset_name):
            os.mkdir("car_data/%s" % dataset_name)
        try:
            self.original_start_indices = np.load("car_data/%s/original_start_indices.npy" % dataset_name).tolist()
            self.original_odometry_data = np.load("car_data/%s/original_odometry_data.npy" % dataset_name).tolist()
            self.filtered_start_indices = np.load("car_data/%s/filtered_start_indices.npy" % dataset_name).tolist()
            self.filtered_odometry_data = np.load("car_data/%s/filtered_odometry_data.npy" % dataset_name).tolist()
        except:
    ...
    def save_dataset(self, p=None):
        ...
        if len(self.current_data) > 500:
            ...
            self.current_data = []
            np.save("car_data/%s/original_start_indices" % self.dataset_name, np.asarray(self.original_start_indices))
            np.save("car_data/%s/original_odometry_data" % self.dataset_name, np.asarray(self.original_odometry_data))
            np.save("car_data/%s/filtered_start_indices" % self.dataset_name, np.asarray(self.filtered_start_indices))
            np.save("car_data/%s/filtered_odometry_data" % self.dataset_name, np.asarray(self.filtered_odometry_data))
